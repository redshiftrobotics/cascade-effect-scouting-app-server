...
if(!settingsJson.database) {
	// prevents key errors when setting defaults
	settingsJson.database = {};
}
...
if (settings.database.engine === 'sqlite') {
	...
	if (settings.database.name === ':memory:'
	    && ! settings.dev) {
		console.error('[fail] Refusing to run with database \':memory:\' in a production environment (i.e. when the \'dev\' settings key is not set to true).');
		process.exit(1);
	}
} else {
	console.error('[fail] Unrecognized database ' + settings.database.engine + '.');
	process.exit(1);
}
